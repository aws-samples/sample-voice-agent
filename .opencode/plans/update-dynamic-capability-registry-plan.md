# Plan: Update Dynamic Capability Registry to Use Strands SDK Native A2A

## Context

After evaluating pipecat PR #2610 (StrandsAgentsProcessor) and the Strands Agents SDK 1.0 A2A capabilities, we decided on **Option A: Strands for remote capability agents only**. The Pipecat pipeline LLM stays as `AWSBedrockLLMService` to preserve latency, filler phrases, and observer compatibility. Strands SDK is used to build the remote capability agents and for A2A client communication.

Additionally, we decided to drop custom `systemPromptFragment` extensions and stay close to the A2A standard. The LLM will rely on standard tool descriptions (auto-generated from Strands `@tool` docstrings) to reason about when/how to use each capability.

## Files to Modify

### 1. `docs/features/dynamic-capability-registry/plan.md` -- Full Rewrite

Replace the entire file with the updated plan below. Key changes from the previous version:

- **Overview**: Drop "composes the system prompt dynamically from agent card fragments". Add latency constraint explanation and Strands SDK role.
- **Architecture diagram**: Updated to show Strands A2AServer on capability agents, A2AAgent on voice agent side, and note that pipeline is UNCHANGED.
- **Architecture Decisions**: 
  - #4: Remove `systemPromptFragment` extension -- standard A2A Agent Cards only
  - #7: Replace prompt composition with model-driven tool use
  - #10: Replace "port BaseAgent from workshop" with "use Strands SDK native A2A"
  - Added #11 (A2A client via Strands `A2AAgent`), #13 (rollout), #14 (keep `AWSBedrockLLMService`)
- **Research Prerequisites**: Added R6 (Strands A2AAgent response mapping). Removed AgentCardExtended references.
- **Phase 1**: Dropped 1.1 (custom A2A client) entirely -- Strands `A2AAgent` handles it. Dropped `types.py`. Simplified registry to cache `A2AAgent` instances.
- **Phase 2**: Tool adapter uses `A2AAgent.invoke_async()` instead of raw HTTP. Dropped prompt fragment composition. `_register_capabilities()` returns only tool specs, no prompt fragments.
- **Phase 3**: Dropped 3.3 (base agent package) -- replaced by `strands-agents[a2a]`. No `capability-agent-base/` directory.
- **Phase 4/5**: Agents defined with `@tool` + `Agent()` + `A2AServer()` pattern (~50 lines each). Explicit note that `@tool` docstrings ARE the tool descriptions.
- **Phase 5.2**: Transfer and diagnostics explicitly stay local (not "research required").
- **File structure**: Simpler -- no `capability-agent-base/`, no `a2a/client.py`, no `a2a/types.py`.
- **Dependencies**: Added `strands-agents[a2a]>=1.15.0`.
- **Effort estimates**: Reduced from ~15 days to ~9 days (risk-adjusted ~11 days from ~18 days).
- **Open Questions**: Added "Will model-driven tool use work well with Haiku?" and "Can A2AAgent return structured data?"
- **Risks**: Added Strands SDK API stability risk, model-driven accuracy risk, response format mapping risk.
- **Progress Log**: Added entry for this revision.
...
- **Self-describing:** Standard A2A Agent Cards at `/.well-known/agent.json` declare skills and schemas.
...
| 4 | **Self-description** | Standard A2A Agent Cards at `/.well-known/agent.json` | Auto-generated by Strands `A2AServer` from agent tools. No custom extensions -- stays A2A-compliant. |
...
- [ ] **R1:** Confirm `strands-agents[a2a]` package provides `A2AServer`, `A2AAgent`, and auto-generates Agent Cards from `@tool` definitions. Pin version (currently `>=1.15.0`).
...
- [ ] **R6:** Validate that `A2AAgent.invoke_async()` response can be mapped to a dict suitable for Pipecat's `result_callback()`. Confirm response parsing (text parts, data parts, artifacts).
...
- [ ] Create `backend/voice-agent/app/a2a/tool_adapter.py`
  ...
  - `extract_bedrock_tool_spec(skill: AgentSkill) -> dict`
    - Maps A2A `AgentSkill` (name, description, input schema) to Bedrock `toolSpec` format
    - Uses skill description as tool description
    - Maps skill input schema to Bedrock parameter schema
...
- [ ] Directory: `backend/agents/knowledge-base-agent/`
  ...
  - `requirements.txt`: `strands-agents[a2a]>=1.15.0`, `boto3`
...
- [ ] Verify Agent Card auto-generated at `/.well-known/agent.json` (served by Strands A2AServer)
...
- [ ] Note: `strands-agents[a2a]>=1.15.0` dependency for capability agents
...
- `strands-agents[a2a]>=1.15.0` -- Strands SDK with A2A support (for capability agents + `A2AAgent` client)
- `a2a` Python package -- Transitive dependency via strands-agents[a2a]
- Workshop code at `/workshops/inter-agent-systems-with-strands-agents-amazon-bedrock-mcp-and-a2a` -- reference patterns for CloudMap discovery
...
backend/
  ...
  agents/                             # Individual capability agents (Strands + A2AServer)
    knowledge-base-agent/
      ...
      Dockerfile
      requirements.txt                # strands-agents[a2a], boto3
    crm-agent/
